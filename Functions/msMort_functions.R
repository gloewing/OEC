...
## 
# expected counts from Rolando github

Xgen <- function(counts,
                 exclude = NULL,
                 trend.knots.per.year = 2, # 2 -- 1 knot at each extreme and 1 interorior (3 knots)
                 harmonics = 2,
                 frequency = NULL,
  ...
  # compute knots
  #### Rolando told me to comment this out 8/13/20
  ...
  x_t <- splines::ns(tt, df = trend.knots.per.year, intercept = FALSE)# no intercept because in model #knots = knots, intercept = TRUE)
  i_t <- 1:ncol(x_t)
