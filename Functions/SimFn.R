...
multiStudySimNew <- function(sampSize = 400,
                          covariate.var = 10, # scales the variance of the MVNormal that generates the true means of the covaraites
    ...
    # numRF <- num.studies * (num.covariates + 1 - length(fixed.effects)) # total number of draws of random effect random variables
    ...
    #******************************
    clustBeta <-  matrix(0, nrow = num.studies, ncol = num.covariates + 1)
    ...
    if( !is.null(clusters_beta) & clusters_beta < num.studies ){
    
        # if clusters then make matrix
        num.clusters <- length( unique( clusters_beta ) ) # numnber of unique clusters
        
        #numRF <- clusters_beta * ( num.covariates + 1 - length(fixed.effects) ) # total number of draws of random effect random variables
        ...
        clustMat <- mvrnorm(num.clusters, 
            ...
            #                 rnorm(numRF, 
            #                       mean = 0,
            #                       sd = sqrt(cluster.beta.var)
            #                       ), 
            #                nrow = num.clusters, 
            #                ncol = num.covariates + 1)
            # 
    ...
    # full coefficients
    betas <- beta.matrix + random.effects + clustBeta
