...
# sims6 <- cbind(1, expand.grid(c(0.0001, 0.01, 1), c(0.01, 0.5, 1.5), c(c(3, 6)), #  0.1,
#               c("zero", "cvCF") , c(TRUE), c(TRUE),
#               c(300), c(150))) # "cvCF"
# colnames(sims6) <- c("simNum", "betaVar", "xVar", "clusts", "tn",
#                      "etaSet", "scaleInd", "nLow", "nHigh")
# # sims6 <- sims6[sims6$etaSet != sims6$scaleInd,] # already did the ones where they are equal
# # sims6 <- sims6[-seq(1,12),]
# write.csv(sims6, "param22", row.names = FALSE)

sims6 <- read.csv("param22")


save.folder <- "/n/home12/gloewinger/sims23"
...
# sim parameters
totalStudies <- 6 # need number that is divisible by 2, 4, 8 for cluster purposes and structure of 
clusts <-  sims6[runNum, 4] # totalStudies
betaVar <- sims6[runNum, 2] 
covVar <- sqrt( sims6[runNum, 3] )
trainStudies <- 1:(totalStudies - 1)
K <- length(trainStudies)
num.trainStudy <- length(trainStudies)
sample.size <- 100
numCovs <- 20# 20 # number of covaraites
ZeroCovs <- seq(10,20)
totalTestSz <- 150 # number including training test set and actual test set
testTrain <- 50
nLow <- sims6[runNum, 8]  # samp size lower
nHigh <- sims6[runNum, 9]  # samp size upper
...
oecTune <- as.character( sims6[runNum,5] ) 
stdTn <- as.character( sims6[runNum,5] ) # this is hyperparameters for standard stacking
...
stackTune <- "cv" 
stackTuneInd <- TRUE
scaleInd <- sims6[runNum, 7] # scale covaraites
glmnetOpt <- FALSE # 1
itrs <- 2 # number of runs of OEC
lowLimTune <- FALSE
TScv <- FALSE # tune stacking with caret TS instead of cv.glmnet
...
psiH <- Inf
reTune <- FALSE
etaSet <- sims6[runNum, 6]
orderRandom <- FALSE # whether to randomize blocks in block coordinate descent
...
lamb <- 0.5
fileNm <- paste0("oecSpc2.",OECeta, ".etTn_", etaTuneInd, "_", etaTune, 
                "_stTn_", stackTune, "_oecTn_", oecTune,
                 "_stTnIn_", stackTuneInd, "_stCV_", stackCV,
               "_etaSt_", etaSet,
                "_psi_", psiL, ".", psiH,
                ...
                "_glm_", glmnetOpt,
                ".pca.",pcaInd,
                  "_p", numCovs, "_n_", nLow, ".", nHigh, #min(sample.size), "_", max(sample.size),
                 "_eps_", epsLow, ".", epsHigh, "_smpSzW_", sampSzWeight, 
                "_bVar_", betaVar, "_xVar_", round( covVar, 2), "_clst_", clusts,
                 "_reTn_", reTune, "_K_", K, "_s_", simNum)
print(fileNm)
...
if(etaSet == FALSE){
    ...
    etas <- unique( sort( c(0.001, 0.01, seq(0.05, 0.95, 0.05), 0.99, 0.999, 
                  exp(-seq(5,40, length = 40)),
                              1 - exp(-seq(5,40, length = 40))
                      ) ) )
}
...
results <- foreach(iterNum = 1:totalSims, .combine = list, .multicombine = TRUE) %dopar%{
    ...
    library(glmnet)
    library(dplyr)
    resMat <- matrix(nrow = totalSims, ncol = 67)
    colnames(resMat) <- c("merge", "avg", "stacking", "stacking_zeroOut", 
                          ...
                          "oec_countryWindow",
                          "oec_SpecPCA",
                          "gen0", "gen0Avg", "gen0Std", "gen0_2", "gen0Avg_2", "gen0Std_2",
        ...
        full <- as.data.frame( multiStudySim(sim.num = simNum,
                                                ...
                                                beta.mean.range = 2, # true means of hyperdistribution of beta are drawn from a unif(-beta.mean.range, beta.mean.range)
        ...
        # remove last training study (arbitrarily) and replace with specialist training set
        rmIndx <- test_country - 1 # arbitrarily choose the one before test set
        rmCountryIndx <- which(full$Study == rmIndx)
        test2 <- full[rmCountryIndx,] # rows of exchangable study
        full <- full[-rmCountryIndx,] # remove rows of study right before training study
        rm(rmCountryIndx)
        
        # add portion of test study to be specialist training set -- add to Merged; the rest, set aside as test set
        testIndx <- which(full$Study == test_country)
        sampIndx <- sample.int(length(testIndx), testTrain) # take random sample of rows to be specialist training set 
        
        testIndx <- testIndx[-sampIndx] # rows of test set
        test <- full[testIndx,]
        ...
        full$Study[specIndx] <- rmIndx
        
        # rename test_country and study label
        test_country <- rmIndx
        test$Study <- rmIndx
        
        rm(specIndx, testIndx)
        ...
        predsVecOEC <- rowMeans( pMat2 ) # average predictions from the different runs
        
        resMat[iterNum, 12] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
...
#################################################################################################### 
        ...
        for(it in 1:length(etaVec) ){
            ...
            # if first pass then initialize on generalist stacking
            if(it == 1){
                B_itr <- betas
                ...
                resMat[iterNum, 59] <- sqrt( mean( (test2$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 60] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 57] <- sqrt( mean( (test2$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 58] <- min(oecRidgeWS$objImp) # best objective achieved
        resMat[iterNum, 65] <- OECeta
        ...
        for(it in 1:length(etaVec) ){
            ...
            if(it == 1){
                ...
                resMat[iterNum, 63] <- sqrt( mean( (test2$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 64] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 61] <- sqrt( mean( (test2$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 62] <- min(oecRidgeWS$objImp) # best objective achieved
        ...
        predsVecOEC <- rowMeans( pMat2 ) # average predictions from the different runs
        resMat[iterNum, 13] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
...
####################################################################################################                
        ...
        for(it in 1:length(etaVec) ){
            ...
            if(it == 1){
                ...
                resMat[iterNum, 43] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 44] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 41] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 42] <- min(oecRidgeWS$objImp) # best objective achieved
        ...
        for(it in 1:length(etaVec) ){
            ...
            if(it == 1){
                ...
                resMat[iterNum, 47] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 48] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 45] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 46] <- min(oecRidgeWS$objImp) # best objective achieved
        resMat[iterNum, 66] <- OECetaSpec
        ####################################################################################################    
...
####################################################################################################  
        ...
        for(it in 1:length(etaVec) ){
            ...
            if(it == 1){
                ...
                resMat[iterNum, 51] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 52] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 49] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 50] <- min(oecRidgeWS$objImp) # best objective achieved
        ...
        for(it in 1:length(etaVec) ){
            ...
            if(it == 1){
                ...
                resMat[iterNum, 55] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
                resMat[iterNum, 56] <- min(oecRidgeWS$objImp) # best objective achieved
            }
        ...
        resMat[iterNum, 53] <- sqrt( mean( (test$Y - predsVecOEC1)^2 ) )
        resMat[iterNum, 54] <- min(oecRidgeWS$objImp) # best objective achieved
        resMat[iterNum, 67] <- OECetaSpec0
        ####################################################################################################        
        ...
        ####################
        nfolds <- itrs
        foldsList <- createFolds(factor(full$Study), k = nfolds) # make folds with balanced studies
        
        pMat <- matrix(ncol = nfolds, nrow = nrow(test2) ) # store predictions in each column 
        
        studs <- full$Study # save original study labels
        
        for(it in 1:nfolds){
            
            full$Study <- studs # set to original
            
            indx1 <- foldsList[[it]] # index of set used for stacking portion
            full$Study[indx1] <- K + 1 # set index to K+1 (arbitrarily)
                
            # warm start with these "fake" study labels
            warmRes <- ridgeWS(data = full, 
                               tuneParam = c(tuneParamOEC$lambda, 0.1), # add arbitary tuning parameter -- it wont be used
                               stackParam = stackParamOEC, 
                               nnlsInd = TRUE,
                               stackTune = TRUE, 
                               modelStandardize = TRUE,
            ...
            oecRidgeWS <- ridgeAltSpec0(data = full, 
                                        betaStart = warmRes$beta, 
                                        wStart = warmRes$w,
                                        lambdaVec = c(tuneParamOEC$lambda, 0.1), # add arbitary tuning parameter -- it wont be used
                                        mu = stackParam, #stackParamOECSpec0 #
                                        Stackindx = indx1, # rows corresponding to test country
                                        nnlsInd = TRUE,
                                        tol = 0.001,
                                        objCriteria = TRUE,
                                        eta = OECeta, # OECetaSpec0, #
                                        dataSplit = 1,
                                        projs = 0,
                                        Avg = FALSE,
                                        wUpdate = "glmnet",
            ...
            pMat[,it] <- oecRidgePred(data = test2[,-c(1,2)],
                                      mod = oecRidgeWS)
            # mod = oecRidgeWS)
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        #}
        resMat[iterNum, 35] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        #}
        resMat[iterNum, 36] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        ##############################################################
        ...
        for(it in 1:nfolds){
            ...
            # standardize
            # ++++++++++++++++++++++++++++++++++++++++++++++++
            nFull <- length(indx1) # sample size of merged
            
            # scale Covaraites
            means <- colMeans( as.matrix(full[indx1, -c(1,2)]) )
            sds <- sqrt( apply( as.matrix(full[indx1, -c(1,2)]), 2, var) *  (nFull - 1) / nFull )  # use mle formula to match with GLMNET
            
            #
            for(column in 3:ncol(test2) ){
                # center scale
                full[, column ] <- (full[, column ] - means[column - 2]) / sds[column - 2]
                test2[, column ] <- (test2[, column ] - means[column - 2 ]) / sds[column - 2]
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        resMat[iterNum, 37] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        ##############################################################
        
        #####################
        # Generalist zero out but use etas/mufrom specialist
        #####################
        full$Study <- studs # set to original
        nfolds <- itrs
        foldsList <- createFolds(factor(full$Study), k = nfolds) # make folds with balanced studies
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        #}
        resMat[iterNum, 38] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        #}
        resMat[iterNum, 39] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        ##############################################################
        
        ##################################################
        ####################
        # Generalist Zero Out with etas and mu from specialist
        ####################
        nfolds <- K
        full$Study <- studs # set to original
        foldsList <- createFolds(factor(full$Study), k = nfolds) # make folds with balanced studies
        ...
        predsVecOEC <- rowMeans( pMat ) # average predictions from the different runs
        #}
        resMat[iterNum, 40] <- sqrt( mean( (test2$Y - predsVecOEC)^2 ) )
        # apply(pMat, 2, function(x) sqrt( mean( (test2$Y - x)^2 ) ) )
        ##############################################################
