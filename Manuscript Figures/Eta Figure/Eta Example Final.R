...
testYears <- 2003:2019 # 2010:2019 #chosen because many countries in March 2012 have just over 2 years worth of data, and
...
dataNm <- "world-mort.csv" # "world-mort_old.csv"
...
etaTuneInd <- TRUE
tuneParamZero <- FALSE 
etas <- sort( c(0.001, 0.01, seq(0.05, 0.9, 0.05)) ) 
stackTol <- 1e-9 
oecTune <- "zero"
# added in
stdTn <-  "zero"
...
# zero out
################
etasVec <- c(0.1, 0.5, 0.75,# 0.8, #0.825, 0.85, 0.875,
             0.9, 0.99, 0.9925, 9.995, 0.9975, 0.999, 0.99925, 0.9995, 0.9997, 0.9999, OECetaSpec0) # 0.9275, 0.93, 0.9325, 0.933, 0.934, 0.935, 0.936, 0.9375, 0.94, 0.95,
...
write.csv(etaMat0, paste0("~/Desktop/Research Final/Mortality/Figures/Final Figures/Eta Plots/etaData_cnt_", cnt, "_itr_", iterNum) )
par(mfrow = c(1,1))
############################################################################

############################################################################
setwd("~/Desktop/Research Final/Mortality/Figures/Final Figures/Eta Plots/Eta Examples")

par(mfrow = c(1,1))

jpeg(paste0("etaPlot: ", "cnt: ", cnt, " iterNum: ", iterNum,".jpg"), width = 600, height = 500)
# country specific model
plot(c(full$Y[trainIndx], test$Y),
     ...
     pch = 19,
     lwd = 0.1,
     main = paste0("cnt: ", cnt, " iterNum: ", iterNum) )
